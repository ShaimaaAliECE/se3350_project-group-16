        ...
        if (Array.isArray(element[0]) && index === 0) {
          return (
            <div key={nextId()} className="nums-outer">
              ...
              <div className="picked-nums">
                {mergedArray.map((num, index4) => {
                  return (
                    <div key={index4} style={{ marginLeft: "15px" }}>
                      {num}
                    </div>
                  );
                })}
...
const Steps2 = (props) => {
  ...
  function checkMerge(merge) {
    ...
    if (
      ...
      JSON.stringify(compareData[compareData.length - 1])
    ) {
      sfx.right.play();
      setColor(true);
      setResult("CORRECT");
  ...
  function changeStep(newValue) {
    if (props.contents.length > 0) {
      if (newValue < index1 && newValue >= 0) {
        ...
        setStep(
          props.contents[index3 - 1][index2][
            props.contents[index3 - 1][index2].length - 1
          ]
        );
      ...
      if (
        index3 === props.contents.length - 1 &&
        index1 === props.contents[index3][index2].length - 1
      ) {
    ...
    <div className="steps-header">
      <div className="steps-body">
        ...
        <button
          ...
          disabled={index1 <= 0 && index3 === 0 && index2 === 0 ? true : false}
        >
          Previous Step
        </button>
        <button
      ...
      <div style={{ color: toggleColor ? "green" : "red" }} id="RW">
        {result}
      </div>
        ...
        <div className="steps-body">
          ...
          <div>
            <Numbers
              data={data}
              toggle={toggleNum}
              checkSelect={checkSelection}
              selectColor={toggleColor}
              sendMerge={checkMerge}
            ></Numbers>
          </div>
        </div>
      </div>
